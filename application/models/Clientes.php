...
class Clientes extends CI_Model {
    ...
    public function actualizarCliente($token, $id, $nombre, $apellido, $email, $telefono, $password) {
        ...
        if (!in_array($this->jwt->getProperty($token, 'perfil_nombre'), array('Administrador', 'Cliente'))) {
        ...
        if (in_array($this->jwt->getProperty($token, 'perfil_nombre'), array('Administrador'))) {
            ...
            return $response;
        } else if (in_array($this->jwt->getProperty($token, 'perfil_nombre'), array('Cliente'))) {
    ...
    public function getDatosUsuario($token, $id = null) {
        ...
        $esAdmin = in_array($this->jwt->getProperty($token, 'perfil_nombre'), array('Administrador'));
